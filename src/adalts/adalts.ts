...
export class RenewalList implements IRenewalList {
    [resource: string]: any;
}
...
export class Logging {
    ...
    static log: ILogFunction = (m: string) => { console.log(m); }
}
...
export class AuthenticationContext implements IAuthenticationContext {
    ...
    */
    ...
    constructor(cfg: IConfig) {
        ...
        (window as IOAuthWindow).callBackMappedToRenewStates = new CallbackMap<IRequestCallback>();
        (window as IOAuthWindow).callBacksMappedToRenewStates = new CallbackMap<Array<IRequestCallback>>();
    ...
    private _extractIdToken(encodedIdToken: string): IUserProfile {
    ...
    private _createUser(idToken: string): IUser {
        var user: User = null;
    ...
    registerCallback(expectedState: string, resource: string, callback: IRequestCallback): void {
        ...
        if (!(window as IOAuthWindow).callBacksMappedToRenewStates[expectedState]) {
            (window as IOAuthWindow).callBacksMappedToRenewStates[expectedState] = [];
        }
        (window as IOAuthWindow).callBacksMappedToRenewStates[expectedState].push(callback);
        if (!(window as IOAuthWindow).callBackMappedToRenewStates[expectedState]) {
            (window as IOAuthWindow).callBackMappedToRenewStates[expectedState] = (message: string, token: string) => {
                for (var i = 0; i < (window as IOAuthWindow).callBacksMappedToRenewStates[expectedState].length; ++i) {
                    (window as IOAuthWindow).callBacksMappedToRenewStates[expectedState][i](message, token);
                }
                this._activeRenewals[resource] = null;
                (window as IOAuthWindow).callBacksMappedToRenewStates[expectedState] = null;
                (window as IOAuthWindow).callBackMappedToRenewStates[expectedState] = null;
            };
        }
    }
        ...
        if (parameters) {
            ...
            if (parameters.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION) ||
                ...
                if (!requestInfo.stateMatch && window.parent && (window.parent as IOAuthWindow).AuthenticationContext) {
                    var statesInParentContext = ((window.parent as IOAuthWindow).AuthenticationContext as AuthenticationContext)._renewStates;
